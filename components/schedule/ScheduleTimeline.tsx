...
const SPAN = SCHEDULE_END_HOUR - SCHEDULE_START_HOUR;
const HOURS = Array.from({ length: SPAN + 1 }, (_, i) => SCHEDULE_START_HOUR + i);
...
function pct(hour: number) {
  return ((hour - SCHEDULE_START_HOUR) / SPAN) * 100;
}

export function ScheduleTimeline({ pools }: { pools: SchedulePool[] }) {
  const inventory = getInventory();
  const invByType = Object.fromEntries(inventory.map((i) => [i.gpuType, i]));
    ...
    <div className="overflow-x-auto">
      <div className="min-w-[720px]">
        ...
        <div className="divide-y divide-border">
          {pools.map((pool) => {
            const inv = invByType[pool.gpuType];
            return (
                ...
                <div className="flex w-40 shrink-0 flex-col justify-center px-4 py-3">
                  ...
                  {inv && (
                    <span className="tabular text-2xs text-text-muted">
                      {inv.allocated}/{inv.total} allocated
                    </span>
                  )}
                </div>
